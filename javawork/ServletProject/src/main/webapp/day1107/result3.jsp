...
<h5>프로젝트의 url 주소 얻기</h5><!-- 부득이하게 java 뭐시기 -->
...
<c:if test="${param.age>=20}">
	<h2 style="colot:blue">당신은 성인입니다 </h2>
</c:if>
<c:if test="${param.age<20 }">
	<h2 sytle="color:red">당신은 미성년자입니다</h2>
</c:if>
<hr>
<c:choose>
	<c:when test="${param.nara=='일본'}">
		<h3 style="color:green">일본의 거리는 아름답다</h3>
	</c:when>
	<c:when test="${param.nara=='캐나다'}">
		<h3 style="color:magenta">캐나다의 메이플시럽은 아름답습니다</h3>
	</c:when>
	<c:when test="${param.nara=='미국'}">
		<h3 style="color:magenta">미국의 피자는 아름답습니다</h3>
	</c:when>
	<c:otherwise>
