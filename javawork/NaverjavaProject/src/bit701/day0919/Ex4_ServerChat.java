...
public class Ex4_ServerChat extends JFrame implements Runnable{
	...
	Vector<ClientMember>listMenber=new Vector<ClientMember>();
...
//				e.printStackTrace();
		...
		public void run() {
			// TODO Auto-generated method stub
//			super.run();
			//언제 클라이언트들이 메세지를 보낼지 모르므로 while 문을 열어놓는다 
			while(true)
			{
				//클라이언트들이 보낸 메세지를 읽는다
				try {
					...
					case "1":
						...
						for(ClientMember mem:listMenber)
						{
					...
					case "2":
						...
						//접속한 모든 클리이언트들한테 누가 어떤 메세지를 보냈는지 알린다 
						for(ClientMember mem:listMenber)
						{
