...
public class Ex2_KeyInput {
	...
	public static void main(String[] args) {
		...
		//문자열로 읽은 후 숫자로 변환하면 다음에 문자열이 입력 될 경우에도 문제가 발생하지 않는다
		...
		//if 문으로 해도 되고 조건(삼항)연산자(조건식?참 일때 값:거짓일때값) 로 해도 된다 
//		String msg=age>=20?"성인":"미성년자";
		
		
		//if문으로 할 경우
		String msg="";
		//지역변수는 자동 초기화가 안되고 쓰레기값이 들어 있으므로
		//값이 안 들어갈 경우를 대비해서 초기값을 지정하는 것이 좋다
		//지역변수는 그 구역을 빠져나가게 되면 자동 소명 되므로 사용이 불가능하다
		
		if(age>=20) {
			//한 문장만 쓸 경우 {} 구역 서렁 생략 가능하지만 두 문장 이상일 경우는 반드시 구역 설정{})
			msg="성인";
		}else {
			msg="미성년자";
		}
