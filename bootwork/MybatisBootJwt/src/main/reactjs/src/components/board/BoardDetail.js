...
const BoardDetail = () => {
    ...
    useEffect(() => {
        axios.get(`/board/detail?num=${num}`).then((res) => {
            setData(res.data);
        });
    }, []);
        ...
        <div style={{ padding: "30px" }}>
            {data && (
                <div>
                    ...
                    {data.photo.length > 0 ? (
                        <div>
                            <img alt='' src={STORAGE_URL + data.photo} style={{ maxWidth: "400px" }} />
                            <br />
                        </div>
                    ) : (
                        ""
            ...
            <br />
            <hr />
            <Button variant='outlined' color='info' onClick={() => navi("/board/list")}>
                목록
            </Button>
            &nbsp;&nbsp;
            <Button variant='contained' color='secondary' onClick={() => navi("/board/form")}>
                글쓰기
            </Button>
        </div>
    );
