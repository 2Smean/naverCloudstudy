...
import errimg from '../../image/noimage.png';
...
const MemberForm = () => {
    ...
    const [open, setOpen] = useState(false); // 다이얼로그 open/close 담당하는 변수 
    const [openPostcode, setOpenPostcode] = useState(false); //카카오 주소록 open/close 담당하는 변수 
    const [idcheck, setIdcheck] = useState(false); // 아이디 중복확인을 체크하기 위한 변수
    ...
    // 중복 확인 버튼 이벤트 
    const buttonIdCheck = () => {
        // myid 값이 비어 있는 경우 중복확인 API 호출을 하지 않음
        if (!myid) {
            alert("아이디를 입력해주세요");
            return;
        }
    ...
    const saveMemberEvent = () => {
        ...
        // DB 저장
        axios.post("/member/insert", { name, myid, hp, addr, pass })//key value 같아서 생략가능
            .then(res => {
                //멤버 추가 후 어디로 갈지 ? nav 추가 import 
                navi("/member/list");
            })
                    ...
                    <DialogContentText id="alert-dialog-description">
                        {
                            ...
                            <DaumPostcodeEmbed
                                ...
                                defaultQuery='강남대로 24길' //팝업을 열때 검색창의 기본주소 
                            />
                        }
                    </DialogContentText>
                </DialogContent>
                    ...
                    <tr>
                        ...
                        <td className='input-group'>
                            <input type='text' className='form-control'
                                ...
                                onChange={(e) => {
                                    setIdcheck(false);// 아이디 입력시 중복체크 버튼 다시 눌러야함
