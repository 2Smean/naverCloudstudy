...
public class TestController {
	...
	// @RequestMapping("/") //기본이 get방식이므로 get방식으로 호출이 됨 :초기버전부터 모두 가능한 어노테이션
	@GetMapping("/") // 스프링 버전 5부터 가능한 어노테이션
	public String home(Model model) {
		model.addAttribute("message", "Hava a Good Time!!!!");// request 에 저장
...
//	@GetMapping("/shop/list.do") //  /shop.list.do 만 호출 가능
	@GetMapping("./shop/{path}") // @Pathcariable("path1") : path1 으로 넘어온 url 값을 읽는다
...
//	@GetMapping("/board/update")
	@GetMapping({"/board/update","/stu/form/test1"}) //두개의 매칭이 같은 곳으로 포워드한다
