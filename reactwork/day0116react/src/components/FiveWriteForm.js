...
function FiveWriteForm({ onSave }) {
    ...
    //추가 이벤트 
    const onAddEvent = () => {
        onSave({ fname, fphoto, fprice });//원래는 key value 값으로 보내야하는데 key value 값이 같다면{}로 묶어서 보내면 dto 에 넣지 않고 보내도 된다
        //초기화
        setFname('');
        setFprice(0);
    }
    return (
        <div className='input-group'>
            ...
            가격 :
            <input type='text' value={fprice} onChange={(e) => setFprice(e.target.value)}
                style={{ width: '70px' }} />
