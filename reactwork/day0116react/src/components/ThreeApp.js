...
function ThreeApp(props) {
    ...
    //입력 시 inputs 의 해당 멤버변수로 값이 들어가게 하기 위한 이벤트 
    const inputChangeEvent = (e) => {
        const { name, value } = e.target; // 이벤트가 발생하는 태그의 name 과 입력값 얻기 
        setInputs({
            ...inputs, //기존의 값을 펼침연산자로 일단 넣고 
            [name]: value //입력한 태그의 name에 값 변경 ( 나머지는 유지 )
        })
    }

    //배열 추가 이벤트 
    const addArrayEvent = () => {
        //기본 배열 데이타에 묶음변수인 input 추가 
