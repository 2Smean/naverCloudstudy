...
const PersonForm = ({ onSave }) => {
    ...
    const addButtonEvent = () => {
        ...
        //부모 컴포넌트의 메서드를 통해서 입력한 데이타를 보낸다 Props로 받으면 Props.onSave로 받아야한다
    ...
    return (
        <table className='tabel table-bordered' style={{ width: '460px' }}>
            <tbody>
                <tr>
                    <th style={{ width: '100px', backgroundColor: 'pink' }}>이름</th>
                    <td>
                        <input type='text' className='form-control'
                            style={{ width: '130px' }} value={name}
                            onChange={(e) => setName(e.target.value)} />
                    </td>
                    <th style={{ width: '100px', backgroundColor: 'pink' }}>나이</th>
                    <td>
                        <input type='text' className='form-control'
                            style={{ width: '130px' }} value={age}
